 ...
 * Màn hình Đăng Nhập hệ thống KỸ THUẬT & KPI V2.8
 */
...
export class LoginComponent {
  ...
  render() {
      ...
      <div class="w-full max-w-md bg-white rounded-3xl shadow-2xl p-8 border border-slate-100 my-8">
        ...
        <form id="loginForm" class="space-y-5">
          ...
          <div>
            ...
            <div class="relative">
              <select id="userSelect" class="w-full pl-10 pr-10 py-3.5 bg-slate-50 border border-slate-200 rounded-xl font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all cursor-pointer">
          ...
          <div>
            ...
            <input type="password" id="passwordInput" placeholder="Nhập mật khẩu..." class="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all">
          </div>
    ...
    // Lắng nghe sự kiện người dùng bấm đăng nhập
    document.getElementById('loginForm').addEventListener('submit', (e) => {
      e.preventDefault();
      const selectedUser = document.getElementById('userSelect').value;
      if (!selectedUser) {
        alert("Vui lòng chọn tài khoản đăng nhập!");
        return;
      }
      ...
      // Gọi hàm chuyển sang giao diện chính
